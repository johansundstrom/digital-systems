# Introduction to Generative AI (such as ChatGPT, Copilot)

## Objective
The objective of this lab is to introduce you to generative AI and its capabilities. By the end of this lab, you will understand how to interact with an AI-tool, explore its features, and identify its practical applications.

## Prerequisites
- Basic knowledge of text-based communication.
- A device with internet access and a browser.

## Definitions used here 
- **ChatGPT** - developed by OpenAI
- **Copilot** - developed by Microsoft (highly integrated with Microsoft products like Word or Excel)
- **Prompt** -  A prompt is a question, command, or statement used to interact with a large language model (LLM) to produce the intended output
- **Large language model** - LLMs are language models with many parameters, and are trained with self-supervised learning on a vast amount of text
- **Output** - The AI-generated response based on the input

## Useful links

https://www.intelogy.co.uk/blog/easy-copilot-prompt-examples-you-must-try-to-get-started/ 
https://support.microsoft.com/en-us/topic/learn-about-copilot-prompts-f6c3b467-f07c-4db1-ae54-ffac96184dd5

---

## Introduction to Generative AI

- It is designed to generate text, answer questions, and provide contextually relevant responses amongst other.
- The model is trained on extensive datasets to understand and produce natural language effectively.
- Copilot is used across various domains like customer support, education, and programming.
- It can be integrated into applications to enhance user interaction and experience.

---

## Lab Sections

### 1. What is Copilot?
- It can generate text, answer questions, and assist with various tasks like coding, writing, and brainstorming.

#### Example task:
- Ask Copilot: *"What are you, and how can you help me?"*
- Observe and note down the response.

---

### 2. Getting Started
1. Open Microsoft 365 Copilot (free tool after MDU-login)
2. Type a simple question or command like:
   - *"Explain Newton's laws of motion."*
   - *"Translate 'Hello' to Spanish."*

#### Task:
- Experiment with two or three queries of your choice.
- Write down the responses and evaluate their usefulness.

---

### 3. Exploring Capabilities
Copilot is versatile and can:
- Generate creative content (e.g., stories, poems, or essays).
- Assist with programming (e.g., code generation or debugging).
- Answer factual questions based on its training data.

#### Task:
- Try the following:
  - Ask Copilot to write a short poem about spring.
  - Provide a simple coding problem (e.g., *"Write a Python function to add two numbers."*).
  - Ask a factual question (e.g., *"What is the capital of France?"*).
- Document the results.

---

### 4. Limitations of ChatGPT
- Copilot may not have up-to-date information.
- It can generate incorrect or nonsensical answers.
- It is not connected to the internet for real-time data.

#### Task:
- Ask Copilot a question about a recent event (e.g., *"What happened at the 2024 Olympics?"*).
- Note how it responds and identify any limitations.

---

### 5. Practical Applications
Copilot can be used in:
- Education: Summarizing concepts, answering questions, or generating study material.
- Programming: Debugging, code suggestions, or learning new languages.
- Writing: Composing emails, articles, or creative content.

#### Task:
- Think of a practical scenario where Copilot could assist you.
- Write a short description of how you would use it.

---

### 6. Final task Submission

#### Context

You are a student, submitting an application for a workposition. Your task is to decribe the academic subject Information Design is in your submission.

#### Goal

You need to find the perfect balance between a too long and a too short description of what the subject Information design embrace. The tone of the text should be relaxed, personal and avoid being percieved as copied. All  responses should be though of as they were written by you

#### Information

Include personal properties like what areas is your challenges are and which you are confident in

#### Prompt guidelines

- Decribe yourself, your ideal situation in the work position and what subjects you are especially interested in
- Obove decription can be many lines of text in several prompts
- Ensure the prompt is actionable, realistic, and easy to follow
- Highlight unique aspects of your skills and your background
- Ask for an output that is well formatted in with divided paragraphs
- Don't forget, perfect balance between a too long and a too short description

Final task

- Summarize your findings from each section of the lab.
- Reflect on how ChatGPT can assist in your personal or professional tasks.
- Submit your work as a document or presentation.
- Send it to johan.sundstrom@mdu.se, use e-mail subject "AI-Intro".

---

## Conclusion
This lab introduced you to ChatGPT's capabilities, limitations, and potential applications. By experimenting with it, you can better understand how to leverage AI tools effectively.

---

Happy Exploring!
